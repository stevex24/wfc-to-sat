...
OUTDIR = Path("experiments/waps_probe/out")
...
def make_training_grid(size: int = 24) -> list[str]:
    """Synthetic PCG-like tile map with biased regions and rare features."""
    grid = []
    ...
    for y in range(size):
        ...
        for x in range(size):
            if x < size * 0.58:
                ...
                ch = "W"          # water: less common

            # Rare mountains in a diagonal ridge
            if abs(x - y) <= 1 and 4 < x < size - 4:
                ch = "M"

            # Rare settlements
            if (x, y) in {(5, 18), (6, 18), (17, 5), (18, 5)}:
                ch = "T"
...
def main() -> None:
    ...
    with open(OUTDIR / "results.md", "w", encoding="utf-8") as f:
        f.write("# Mini-WAPS Probe Results\n\n")
