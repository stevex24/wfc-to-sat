...
def save_stick_grid(grid, path, scale=10):
    image = Image.new("RGBA", (len(grid[0]) * scale, len(grid) * scale))
    pixels = image.load()
    for y, row in enumerate(grid):
        for x, tile in enumerate(row):
            color = (255,255,255,255) if tile == "W" else (24,24,27,255)
            for yy in range(y*scale, (y+1)*scale):
                for xx in range(x*scale, (x+1)*scale): pixels[xx,yy] = color
    image.save(path)
